$(function(){
    ...
    function updateCart() {
        ...
        cart.forEach(item => {
            ...
            <div class="card-body cart-item" data-id="${item.id}" data-price="${item.price}">
                <div class="Pic">
                        <img loading="lazy" src="${item.image}" class="rounded" alt="Product">
                </div>
                    <div class="item">
    ...
    $(document).on("click",".cart_reduce", function(){
        ...
        if(count_el.val() >1){
            ...
            updateLocalStorageCart($(this).closest(".cart-item").data("id"), count);
            calculateTotal();
        }
    });
    ...
    //更新數量
    function updateLocalStorageCart(id, quantity) {
        const cart = JSON.parse(localStorage.getItem('momento_cart') || '[]');
