...
public class OrganizerEmailService {
	...
	public void sendResetPasswordEmail(String toEmail, String token) {
		...
		// 設定內容
		String resetLink = "http://localhost:8080/organizer/reset-password?token=" + token;
	...
	/**
	 * 發送審核結果通知信
	 * @param toEmail 申請者的 Email (雖然我們現在寫死寄給自己，但參數還是要傳進來備用)
	...
	public void sendAuditResultEmail(String toEmail, String organizerName, boolean isApproved, String reason){
		...
		if (isApproved) {
			...
			content.append("<p style='font-size: 16px;'>我們很高興地通知您，您的主辦單位申請已經 <strong style='font-size: 18px;color: green;'>通過審核</strong>！</p>");
			content.append("<br/>");
			content.append("<br/>");
			content.append("<p>您現在可以登入後台，開始建立您的精彩活動以及上架商品。</p>");
			content.append("<br/>");
			content.append("<a href='http://localhost:8080/organizer/login' style='background-color: #4CAF50; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px;'>前往主辦方後台</a>");
		} else {
			//申請駁回
			subject = "【Momento】通知：您的主辦單位申請未通過";
			content.append("<p style='font-size: 16px;'>很抱歉通知您，您提交的主辦單位申請 <strong style='color: red;'>未能通過審核</strong>。</p>");
			content.append("<div style='background-color: #fff3cd; border: 1px solid #ffeeba; padding: 10px; margin: 10px 0; border-radius: 5px;'>");
			content.append("<strong>駁回原因：</strong>").append(reason); // 這裡填入原因
			content.append("</div>");
			content.append("<p>請修正相關資訊後，重新提交申請，謝謝。</p>");
			content.append("<br/>");
			content.append("<a href='http://localhost:8080/organizer/apply' style='background-color: #4CAF50; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px;'>重新申請主辦方</a>");
			}
	...
	private void sendHtmlEmail(String toEmail, String subject, String content){
		try{
			...
			mailSender.send(message);
			System.out.println("HTML信件已經發送到測試信箱，原收件人為：" + toEmail);
		} catch (Exception e) {
